...
export const ImageUploader = ({ onChange, error }) => {
    ...
    return (
        <div className="flex flex-col space-y-2">
            <label className="text-gray-700">Imagen de Perfil</label>
            <div 
                className={`border-2 border-dashed ${ 
                    error ? "border-red-500" : "border-gray-300"
                    } rounded- lg p-4 text-center cursor-pointer hover:bg-gray-50 transition duration-200 ease-in-out`}
                onClick={handleClick} //Muestra la vista previa de la imagen al subirla
            >
                {
                    preview ? (
                ...
                <input //Para poder subir el archivo
                    ref={fileInputRef}
                    type="file" 
                    accept='image/*'
